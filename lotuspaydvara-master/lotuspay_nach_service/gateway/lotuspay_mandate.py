...
async def lotus_pay_mandate_import(context, data):
    ...
    try:
        validate_url = get_env_or_fail(LOTUSPAY_SERVER, 'base-url', LOTUSPAY_SERVER + ' base-url not configured')
        api_key = get_env_or_fail(LOTUSPAY_SERVER, 'api-key', LOTUSPAY_SERVER + ' api-key not configured')
        url = validate_url + f'/mandates/import'
        
        str_data = str(data)
        print(url)
        mandate_context_response = requests.post(url, data=data, auth=(api_key, ''))
        print(mandate_context_response)
        print(mandate_context_response.status_code)
        
        mandate_byte= mandate_context_response.content
        mandate_dict=json.loads(mandate_byte.decode('utf-8'))
        
        
        mandate_context_id = mandate_dict.get('id')
        mandate_result = mandate_context_response.status_code

        print('-------------------------------',mandate_context_id)
        log_id = await insert_logs(url,'LOTUSPAY', str_data, mandate_context_response.status_code, mandate_context_response.content, datetime.now())
